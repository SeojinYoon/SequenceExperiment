...
class Text_st_bundle(St_bundle):
    ...
    def __init__(self, units, ISI_times):
        """
        :param units: Text_st_unit(list)
        :param ISI_times: seconds(list) rest time between previous unit and next unit
        """
        ...
        for i in range(0, len(units)):
            ISIs_stimulus.append(ISI_st_unit(wait_visual_stimulus, showing_time=self.ISI_times[i]))
        self.ISI_units = ISIs_stimulus

class Sound_st_bundle(St_bundle):
    # It consists of many Sound stimulus
    def __init__(self, units, ISI_interval):
        """
        :param units: Sound_st_unit(list)
        :param ISI_times: seconds(list) rest time between previous unit and next unit
        """
        super().__init__(units)

        for i in range(0, len(units)):
            self.ISI_units.append(ISI_st_unit(wait_visual_stimulus, showing_time=ISI_interval))
...
class St_Package:
    """
    This class includes set of bundles(many set of stimulus and inter stimulus rest).
    """
    def __init__(self, bundles, bundle_intervals = [0], bundle_interval_text = "+", start_wait_seconds = 0):
        """
        Initialize package
        
        :param bundles: set of trials(St_bundle)
        :param bundle_intervals: seconds(float), How much do you want to wait between previous bundle and next bundle
        :param bundle_interval_text: stimulus text(string), Visual stimulus when a package ends.
        :param start_wait_seconds: seconds(float), How much do you want to wait when a package starts.
        """
        self.start_wait_seconds = start_wait_seconds
        self.bundles = bundles
        self.bundle_intervals = bundle_intervals

        self.start_unit = ISI_st_unit(wait_visual_stimulus, showing_time = start_wait_seconds)

        interval_units = []
        for i in range(0, len(bundles)):
            remain = i % len(bundle_intervals)
            if bundle_intervals[remain] != 0:
                interval_units.append(BundleInterval_st_unit(bundle_interval_text, showing_time=bundle_intervals[remain]))
...
class Psy_display_manager:
    def __init__(self, intermediater, is_flip_Horiz = False):
        ...
        self.is_flip_Horiz = is_flip_Horiz # Stimulus flip
    ...
    def wait_start(self, ready_keys, stop_keys, iteration = None, load_callback = None, ready_callback = None, stop_exp_callback = None):
        """
        Wait start
        ...
        :param stop_callback: callback function when the psychopy experiment stops.
        """
        ...
        self.show_stimulus(Text_st_unit(wait_visual_stimulus))
        ...
        if keys[0] in ready_keys:
            ...
            if ready_callback != None:
                ready_callback(iteration)
            self.intermediater.wait_start()
            return True
        else:
            if stop_exp_callback != None:
                stop_exp_callback()
            core.quit()
            self.win.close()
    ...
    def show_stimulus(self, stimulus):
        ...
        if isinstance(stimulus, Image_st_unit):
        ...
        elif isinstance(stimulus, Sound_st_unit):
            text = self.visual.TextStim(win=self.win,
                                        text=stimulus.text,
                                        height=stimulus.text_height,
                                        bold=True,
            ...
            text.draw()
            self.win.flip()
            if stimulus.sound != None:
                stimulus.sound.play()
            if stimulus.sound_path == None:
                print(str.format("sound: {0}, showing time: {1}sec", "silence", stimulus.showing_time))
            else:
                print(str.format("sound: {0}, showing time: {1}sec", stimulus.sound_path, stimulus.showing_time))
                
            self.call_stimulus_showing_handler("sound", stimulus.sound_path, stimulus.showing_time)

            self.sound_timer = Direct_fire_timer()
            def stop_sound():
                if stimulus.sound != None:
                    stimulus.sound.stop()

            self.sound_timer.start(stimulus.showing_time, stop_sound)
    ...
    def show_bundle(self, bundle, end_process = None):
        self.show_stimuluses(stimuluses=self.make_stimulus_in_bundle(bundle),
                                 end_process=end_process)
    ...
    def make_stimulus_in_bundle(self, bundle):
        """
    ...
    def make_stimulus_in_pkg(self, pkg):
        ...
        for i in range(0, len(pkg.bundles)):
            ...
            stimuluses += self.make_stimulus_in_bundle(bundle)
            ...
            if Input_interface_manager.device_mouse in self.device_names:
                if self.intermediater is not None and self.intermediater.is_use_event_manager():
                    if self.mouseListener == None:
                        self.mouseListener = psychopy.event.Mouse(visible=False)
    ...
    def wait_blocks(self, blocks, iteration):
        ...
        def end():
            if iteration+1 < len(blocks):
                ...
                self.display_manager.show_stimulus(Text_st_unit(wait_visual_stimulus, showing_time=0))
